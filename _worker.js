...
export default {
	async fetch(request, env, ctx) {
		...
		let 默认反代IP = (`${request.cf?.colo || 'hkg'}.${特征码字典[0]}.${特征码字典[1]}SsSs.nEt`).toLowerCase(), 默认反代兜底 = true;
		if (env.PROXYIP) {
...
async function 处理WS请求(request, yourUUID, url, 反代上下文 = {}) {
	const { socket: serverSock, response } = Deno.upgradeWebSocket(request, { headers: { 'Sec-WebSocket-Extensions': '' } });
	try { /* Deno socket 自动 accept，无需调用 */ }
	catch (_) {}
